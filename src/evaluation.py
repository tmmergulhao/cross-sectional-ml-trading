...
class WalkForwardExpandingBacktester:
    """
...
class WalkForwardSlidingBacktester:
    """
    Sliding-window walk-forward backtester for cross-sectional models, with fixed window size

    At each fold:
      - Training set: all data within the training window
      - Embargo: gap_days of data discarded after train_end
      - Test set: next test_months of trading data
    ...
    def __init__(
        self,
        model: AbstractCrossSectionalModel,
        window_size: int = 84,   # 7 years - size of the sliding training window
        test_months: int = 12,
        step_months: int = 1,
        embargo_days: int = 21,
        trading_days_per_month: int = 21,
    ...
    def run(
        ...
        test_days = self.test_months * self.trading_days_per_month
        step_days = self.step_months * self.trading_days_per_month
        fold_starts = range(0, len(dates), step_days)

        results = []
        for fold, start_idx in enumerate(tqdm(fold_starts, desc=f"Walk-forward [{self.model.name}]")):

            #get training limits
            train_start = dates[start_idx]
            train_end = train_start + pd.DateOffset(months=self.window_size)
            train_end_idx = len(dates[dates <= train_end]) - 1
            ...
            #train-test split
            train = panel[(panel.index <= train_end) & (panel.index >= train_start)]
            test = panel[(panel.index >= test_start) & (panel.index <= test_end)]
